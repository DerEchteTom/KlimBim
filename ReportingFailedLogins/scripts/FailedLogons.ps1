...
# SECTION: Load and prepare Meta
$MetaPath = Join-Path (Split-Path $MetaDataFile) 'reportmeta.json'
$meta     = Load-Meta -Path $MetaPath

# Berechne sicheren Startzeitpunkt für Event-Abfrage
if (-not $meta.LastFailedLogonScan -or $meta.LastFailedLogonScan -eq "") {
    Write-Warning "No previous scan timestamp found in reportmeta.json - using fallback: 24h ago"
    $startTime = (Get-Date).AddDays(-1)
} else {
    try {
        $startTime = Get-SafeTime -ReferenceTime $meta.LastFailedLogonScan
    } catch {
        Write-Warning "Invalid timestamp in reportmeta.json - using fallback: 24h ago"
        $startTime = (Get-Date).AddDays(-1)
    }
}

Write-Host "[INFO] Using start time: $($startTime.ToString('yyyy-MM-dd HH:mm:ss'))"
...
function Get-FailedLogonEvents {
    ...
    foreach ($dc in $DomainControllers) {
        Write-Host "[INFO] Querying $EventLogName on $dc since $StartTime..."
...
Backup-Database
Initialize-Database
$events = Get-FailedLogonEvents -StartTime $startTime
