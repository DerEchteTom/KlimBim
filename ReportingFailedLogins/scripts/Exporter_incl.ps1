# Exporter-Included.ps1 - Failed Logons HTML Exporter (nur JSON-Accounts)
...
# === Optional Filtering (nur JSON-Accounts) ===
$filteredUsers = if ($UseUserFilter) {
    $objects | Where-Object {
        ($excludedUsers -contains $_.SubjectUserName) -or
        ($excludedUsers -contains $_.TargetUserName)
    }
} else {
    $objects
}
...
if ($ShowStep -eq 'HTML') {
    ...
    $targetPath    = Join-Path $reportDir "FailedLogons_INCLUDED_$dateStamp.html"
...
<head>
    <meta charset="UTF-8">
    <title>Failed Logons Report - ONLY JSON Accounts</title>
    <style>
        body {
...
<body>
    <h2>Failed Logons Report - ONLY JSON Accounts</h2>
